...
Write-Host "Setting up Laravel Financial Tracker with Docker (Clean Installation)..." -ForegroundColor Green
...
# Remove Laravel files if they exist (but keep our setup files)
Write-Host "Cleaning Laravel files..." -ForegroundColor Yellow
$filesToRemove = @(
    "app", "bootstrap", "config", "database", "public", "resources", "routes", 
    "storage", "tests", "vendor", "composer.json", "composer.lock", "package.json",
    "artisan", ".env", ".env.example", "phpunit.xml", "vite.config.js"
)
...
# Create Laravel project using Docker in a temporary directory
Write-Host "Creating Laravel project using Docker..." -ForegroundColor Yellow
$tempDir = "laravel_temp"

# Create Laravel in temp directory
docker run --rm -v "${PWD}:/workspace" -w /workspace composer:latest create-project --prefer-dist laravel/laravel $tempDir "10.*"
...
Write-Host "Moving Laravel files to project root..." -ForegroundColor Yellow
