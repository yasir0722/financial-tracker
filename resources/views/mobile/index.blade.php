...
@if($tab === 'transactions')
    ...
    <div class="mobile-panel p-2 mb-3"><div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Date</th><th>Transaction</th><th>Bank / Type</th><th>Impact</th><th></th></tr></thead><tbody>@forelse($transactions as $transaction)<tr><td class="text-nowrap">{{ $transaction->transaction_date->format('d/m/Y') }}</td><td class="description">{{ $transaction->transaction_detail }}</td><td>{{ $transaction->bank?->name ?: '-' }}<br><span class="muted">{{ $transaction->spendingType?->name ?: 'Uncategorized' }}</span></td><td class="text-nowrap {{ $transaction->debit > 0 ? 'impact-negative' : 'impact-positive' }}">{{ $transaction->debit > 0 ? '-' : '+' }} RM {{ number_format((float) ($transaction->debit ?: $transaction->credit), 2) }}</td><td><a class="btn btn-sm btn-outline-primary" href="{{ route('transactions.show', $transaction) }}" title="View"><i class="fas fa-eye"></i></a></td></tr>@empty<tr><td colspan="5" class="text-center muted py-4">No transactions found.</td></tr>@endforelse</tbody></table></div></div>{{ $transactions->links() }}
@elseif($tab === 'monitor')
